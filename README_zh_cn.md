...
graph TB
    ...
    subgraph 存储层 Storage
        ...
        JSON["JsonExportProvider<br/>System.Text.Json<br/>导出/导入"]
        XML["XmlExportProvider<br/>XDocument + Base64<br/>导出/导入"]
        BIN["BinaryStorageProvider<br/>自定义协议 + MemoryMarshal<br/>主存储格式"]
    end
...
| `QuiverVectorAttribute` | `Attribute` | 标记向量字段，指定维度 (`dimensions`)、距离度量 (`metric`) 和是否可空 (`Optional`) |
| `QuiverKeyAttribute` | `Attribute` | 标记实体主键（每个实体有且仅有一个） |
| `QuiverIndexAttribute` | `Attribute` | 配置索引类型及调优参数（可选，默认 Flat） |
| `QuiverDbOptions` | `class` | 全局配置：存储路径、默认度量、WAL 配置等 |
...
| `Optional` | `bool` | `false` | 是否允许向量为 `null`。为 `true` 时，null 向量的实体仍可写入但不加入该字段索引 |
...
- `Optional = false`（默认）：向量为 `null` 时抛出 `ArgumentNullException`
- `Optional = true`：向量为 `null` 时跳过该字段的索引，实体仍正常写入；搜索该字段时不返回此实体
...
> - `QuiverSet.Persistence.cs` — 变更追踪、WAL 回放、持久化支持
...
Quiver 使用**自定义紧凑二进制协议**作为唯一的持久化格式，性能最优、文件体积最小，完全向后兼容 v1.x / v2.x 数据文件。

> JSON 和 XML 仅用于**导出/导入**（参见 [ExportAsync / ImportAsync](#exportasync--importasync)），不参与日常持久化。

### 8.3 导出与导入

`QuiverDbContext` 提供 `ExportAsync` / `ImportAsync` 方法，支持将数据导出为 JSON 或 XML 格式（用于调试、迁移、与第三方系统交换数据）：
...
| 格式 | 实现类 | 可读性 | 文件体积 | 适用场景 |
|------|--------|--------|---------|---------|
| `Json` | `JsonExportProvider` | ✅ 优秀 | 较大 | 调试、与第三方系统交换 |
| `Xml` | `XmlExportProvider` | ✅ 良好 | 较大 | 兼容性需求 |
...
db.Images.Add(new ImageEntity
{
    ...
    FaceEmbedding = null // 无人脸，Optional 字段允许 null
});
...
> ⚠️ **注意**：非 Optional 的向量字段（默认）如果传入 `null`，会抛出 `ArgumentNullException` 并给出明确的提示信息：  
...
| `[QuiverVector(dim, metric)]` | 属性 | ✅ | 标记向量字段（至少一个，类型须为 `float[]`）。`Optional = true` 允许 `null` |
