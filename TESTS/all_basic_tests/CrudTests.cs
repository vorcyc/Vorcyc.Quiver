...
public static class CrudTests
{
    public static async Task RunAsync()
    {
        ...
        Assert(db.Items.Count == 150, "CRUD 后内存数量 150");

        await db.SaveAsync();
        var dbRead = new MyMultiVectorDb(path);
        await dbRead.LoadAsync();

        Assert(dbRead.Items.Count == 150, "持久化后加载数量 150");

        var upserted = dbRead.Items.Find("C000");
        Assert(upserted?.Label == "已更新_0" && upserted.Score > 999, "Upsert 数据持久化正确");

        var untouched = dbRead.Items.Find("C100");
        Assert(untouched?.Label == "原始_100" && untouched.IsActive, "未 Upsert 的数据不变");

        Assert(dbRead.Items.Find("C199") == null, "已删除数据不存在");
