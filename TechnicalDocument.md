...
> **文件格式兼容性**：v3.0.0 完全向后兼容 v1.x 和 v2.x 的二进制数据文件（Magic `QDB\x01` / `QDB\x02`）。WAL 文件及现有的 MemoryMapped arena 文件均可直接加载，无需任何迁移。持久化层仅存储实体属性数据，不涉及度量/索引/相似度算法的元数据，因此架构重构对文件格式零影响。
...
> `VectorStorageMode.MemoryMapped` 向量存储（v2.0.0 新增）与 `EntityCacheMode.LazyPaging` 实体分页（v3.0.0 新增）可同时启用，分别作用于向量数组和实体对象两个内存区域。
...
{
    DatabasePath = "mydata.vdb",
    VectorStorage = VectorStorageMode.MemoryMapped  // 默认 Heap
};
```

要求：`VectorStorageMode.MemoryMapped` 模式必须设置 `DatabasePath`（用于派生 arena 文件路径，如 `mydata.vdb.TypeName.FieldName.vec`）。
...
### 变更五：QuiverDbOptions 新增 VectorStorage / EntityCache 配置

```csharp
public class QuiverDbOptions
{
    // ... 原有属性不变 ...
    public VectorStorageMode VectorStorage { get; set; } = VectorStorageMode.Heap;  // ← v2.0.0 新增（原 MemoryMode）
    public EntityCacheMode EntityCache { get; set; } = EntityCacheMode.FullMemory;  // ← v3.0.0 新增（原 EnableLazyLoading）
}
...
启动时 `QuiverDbOptions.Validate()` 校验：`VectorStorageMode.MemoryMapped` 和 `EntityCacheMode.LazyPaging` 模式均必须设置 `DatabasePath`。
...
| `IVectorStore` 实例 | ❌ | 运行时按 `VectorStorageMode` 创建，不持久化 |

**结论**：v1.x 及 v2.x 的 JSON / XML / Binary / WAL 文件均可被 v3.0.0 直接加载，无需任何数据迁移。
...
| **内存映射向量存储** | 可选 `VectorStorage = VectorStorageMode.MemoryMapped`，向量存储在 OS mmap arena 文件中，零 GC 压力 |
...
| `VectorStorage` | `VectorStorageMode` | `Heap` | 向量存储模式：`Heap`（GC 堆）/ `MemoryMapped`（mmap，零 GC） |
